...
#[tauri::command]
pub fn load_gen_cpp(config_path: &str, subtask_name: &str) -> Result<String, String>{
...
#[tauri::command]
pub fn save_gen_cpp(config_path: &str, desc: &str) -> Result<String, String> {
  let dir_path = json::get_project_directory_with_config_file(config_path);
  let config_json = json::parse(&config_path)?;
  let desc_path = Path::new(&dir_path).join(config_json["desc_dir"].as_str().unwrap());
...
#[tauri::command]
pub fn load_validator_cpp(config_path: &str) -> Result<String, String>{
  let dir_path = json::get_project_directory_with_config_file(config_path);
  let config_json = json::parse(&config_path)?; 
  let desc_path = Path::new(&dir_path).join(config_json["desc_dir"].as_str().unwrap());
  let desc = match std::fs::read_to_string(&desc_path) {
...
#[tauri::command]
pub fn save_validator_cpp(config_path: &str, desc: &str) -> Result<String, String> {
  let dir_path = json::get_project_directory_with_config_file(config_path);
  let config_json = json::parse(&config_path)?;
  let desc_path = Path::new(&dir_path).join(config_json["desc_dir"].as_str().unwrap());
